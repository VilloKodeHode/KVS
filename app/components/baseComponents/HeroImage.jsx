...
export const HeroImage = ({
  ...
  return (
    <>
      ...
      {src === selectedImage && (
        <div className="absolute-center h-full w-full items-center rounded-xl bg-KVS-primary bg-opacity-40 z-10 flex justify-center">
          <figure className="w-[66%] h-[105%] z-50">
  ...
  return (
    <div className="animate-slideInLeft relative z-10 max-w-3xl w-full">
      <div className="backdrop-blur-sm  bg-KVS-primary bg-opacity-40 p-4 ml:p-8 z-10 rounded-3xl shadow shadow-KVS-primary">
        {imageSet === 1 && (
          <>
            <button className={`absolute border-2 animate-appear rounded-xl -bottom-10 right-1/3 ${selectedImage ? "bg-slate-500 border-slate-600" : "bg-KVS-primary border-KVS-secondary"}`} onClick={selectedImage ? null :  () => setImageSet(2)}>
              <IoArrowForward className={`h-12 w-12 z-10  ${selectedImage ? "text-slate-600" : "text-KVS-secondary"}`} />
            </button>
        ...
        {imageSet === 2 && (
          <>
            <button className={`absolute border-2 rounded-xl  animate-appear -bottom-10 left-1/3 ${selectedImage ? "bg-slate-500 border-slate-600" : "bg-KVS-primary border-KVS-secondary"}`} onClick={selectedImage ? null : () => setImageSet(1)}>
              <IoArrowBack className={`h-12 w-12 z-10  ${selectedImage ? "text-slate-600" : "text-KVS-secondary"}`} />
            </button>
